...
namespace cplib
{
	...
	private:
		...
		int ParamsToSystem(const Parameters &inp_params, MY_PORT_SETTINGS &params)
		{
			...
			// *** ВАЖНО: Переводим в сырой (raw) режим ***
			cfmakeraw(&params); // ← ЭТО КЛЮЧЕВАЯ СТРОКА!

			// Теперь вручную настраиваем только то, что нужно
			cfsetispeed(&params, inp_params.baud_rate);
			cfsetospeed(&params, inp_params.baud_rate);

			// Длина данных
			params.c_cflag &= ~CSIZE;
			switch (inp_params.data_bits)
			{
			case 5:
				params.c_cflag |= CS5;
				break;
			...
			// Таймаут
			params.c_cc[VMIN] = 0;
			params.c_cc[VTIME] = (inp_params.timeout > 0) ? ((int32_t)(inp_params.timeout * 10) + 9) / 10 : 0;
	...
	public:
		...
		int Read(std::string &str, double timeout = SERIAL_PORT_DEFAULT_TIMEOUT)
		{
			...
			str.resize(255); // выделяем память

			ret = Read(&str[0], str.size(), &rd);
			if (ret != RE_OK)
				return ret;

			str.resize(rd); // обрезаем до реально прочитанного
			return RE_OK;
		}
